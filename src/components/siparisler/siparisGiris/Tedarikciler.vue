...
export default {
  ...
  computed: {
    ...mapGetters(["servis_adres", "als_fyt", "isNewClicked"]),
  },
  mounted() {
    this.urunList = null;
  ...
  methods: {
    ...
    download() {
      ...
      var temp = [
        { text: "Ürün Adı", style: "tableExample" },
      ...
      this.urunList.forEach((element) => {
        ...
        temp = [
          { text: element.uretimAciklama, style: "tableHeader" },
          { text: element.yuzeyIslem, style: "tableHeader" },
          {
            text: element.en + "x" + element.boy + "x" + element.kenar,
            style: "tableHeader",
          },
          ...
          {
            text: this.formatPrice(element.alisFiyati * element.miktar),
            style: "tableHeader",
          },
        ];
      ...
      var dd = {
        content: [
          ...
          {
            text: "Kasa üzerine yazılacak : " + this.urunList[0].siparisNo,
            fontSize: 7,
          },
      ...
      service
        ...
        .then((res) => {
          if (res.status) {
            const link = document.createElement("a");
            link.href =
              this.servis_adres + "siparisler/dosyalar/IcSiparisExcelCikti";
    ...
    generatePdf() {
      ...
      itemNew.forEach((element) => {
        let val = (element.miktar * element.alisFiyati)
          .toFixed(2)
          .replace(".", ","); //total fiyatın birim ayarı
        ...
        const temp = [
          element.uretimAciklama,
          element.en + "x" + element.boy + "x" + element.kenar,
          element.miktar,
          element.urunbirimAdi,
          val1,
          val,
        ];
        rows.push(temp);
      ...
      doc.text(
        50,
        520,
        "1-Malzeme aynen yukarda belirtildiği gibi tüm detaylara uygun olarak hazırlanmalıdır ."
      );
      doc.setFontSize(8);
      ...
      doc.addImage(
        "https://mekmar-image.fra1.cdn.digitaloceanspaces.com/logo/mekmar-logo.png",
        "PNG",
        5,
        3,
        150,
      ...
      doc.addImage(
        "https://mekmar-image.fra1.digitaloceanspaces.com/other/mekmar-imza.png",
        "PNG",
        50,
        560,
        150,
    ...
    IcSiparisDosyaGonder() {
      service
        .setIcSiparisKayit(this.secimTur.concat(this.urunList))
        .then((veri) => {
          if (veri.Status) {
            const link = document.createElement("a");
            link.href = this.servis_adres + "islemler/dosyalar/pdfciktim";
